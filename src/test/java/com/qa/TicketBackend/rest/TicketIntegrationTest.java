...
//	Timestamp timestamp = new Timestamp(System.currentTimeMillis());
	...
	/*
	 * @Test void testCreate() throws Exception { Ticket newTicket = new Ticket(1L,
	 * "Syntax", "Jon", "syntax", timestamp, "not urgent", "solution1", true,
	 * "example@qa.com", "java"); String requestBody =
	 * this.mapper.writeValueAsString(newTicket); RequestBuilder req =
	 * post("/ticket/createTicket").contentType(MediaType.APPLICATION_JSON).content(
	 * requestBody);
	 * 
	 * ResultMatcher checkStatus = status().isCreated();
	 * 
	 * Ticket savedTicket = new Ticket(1L, "Syntax", "Jon", "syntax", timestamp,
	 ...
	 * 
	 * MvcResult result =
	 * this.mockMVC.perform(req).andExpect(checkStatus).andReturn();
	 * 
	 ...
	 * 
	 * 
	 * Ticket newTicket = new Ticket(2L, "Syntax", "Jon", "syntax", timestamp,
	 * "not urgent", "solution1", true, "example@qa.com", "java"); String
	 * requestBody = this.mapper.writeValueAsString(newTicket); RequestBuilder req =
	 * post("/ticket/createTicket").contentType(MediaType.APPLICATION_JSON).content(
	 * requestBody); this.mockMVC.perform(req);
	 * 
	 * Ticket ticket1 = new Ticket(2L, "Syntax", "Jon", "syntax", timestamp,
	 * "not urgent", "solution1", true, "example@qa.com", "java"); String body =
	 * this.mapper.writeValueAsString(ticket1); ResultMatcher checkStatus =
	 * status().isAccepted(); RequestBuilder req1 =
	 * put("/ticket/updateTicket/2").contentType(MediaType.APPLICATION_JSON).content
	 * (body); MvcResult result =
	 * this.mockMVC.perform(req1).andExpect(checkStatus).andReturn(); }
	 * 
	 * @Test void testDelete() throws Exception { RequestBuilder request =
	 * delete("/ticket/deleteTicket/1");
	 * 
	 * ResultMatcher checkStatus = status().is(200);
	 * 
	 * this.mockMVC.perform(request).andExpect(checkStatus);
	 * 
	 * // RequestBuilder request2 = delete("/remove/4"); // // ResultMatcher
	 * checkStatus2 = status().is(500); // //
	 ...
	 * 
	 * Ticket ticket = new Ticket(1L, "Syntax", "Jon", "syntax", timestamp,
	 * "not urgent", "solution1", true, "example@qa.com", "java"); List<Ticket>
	 * tickets = new ArrayList<>(); String content = this.mockMVC
	 * .perform(request(HttpMethod.GET,
	 * "/ticket/readTickets").accept(MediaType.APPLICATION_JSON))
